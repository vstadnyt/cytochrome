...
#There are 8 values for 4 hemes and 2 dipoles per heme: [Qx_n1, Qy_n1, Qx_p1, Qy_p1, Qx_n2, Qy_n2, Qx_p2, Qy_p2]
...
for excl in exclusions_lst:
    cyt_b6f.append(cytochrome_lib.cyt('cytochrome_b6f.txt',excl))
for i in range(len(exclusions_lst)):
    cyt_b6f[i].read_structure_file()
    cyt_b6f[i].Hamiltonian()
    cyt_b6f[i].D_and_R_strength()
    cyt_b6f[i].spectra_plot()
    
cyt_bc1 = []
for excl in exclusions_lst:
    cyt_bc1.append(cytochrome_lib.cyt('cytochrome_bc1.txt',excl))
for i in range(len(exclusions_lst)):
    cyt_bc1[i].read_structure_file()
    cyt_bc1[i].Hamiltonian()
    cyt_bc1[i].D_and_R_strength()
    cyt_bc1[i].spectra_plot()




x_range_nm = cyt_b6f[0].x_range_nm
...
for i in range(len(exclusions_lst)):
    plt.plot(x_range_nm,np.sum(cyt_b6f[i].specR,axis = 0),linewidth=2)
...
plt.subplot(2,2,2)
for i in range(len(exclusions_lst)):
    plt.plot(x_range_nm,np.sum(cyt_bc1[i].specR,axis = 0),linewidth=2)
#plt.plot(x_range_nm,np.sum(specR_full,axis = 0),linewidth=5)
...
plt.subplot(2,2,3)
for i in range(len(exclusions_lst)):
    plt.plot(x_range_nm,np.sum(cyt_b6f[i].specD,axis = 0),linewidth=2)
#plt.plot(x_range_nm,np.sum(specR_full,axis = 0),linewidth=5)
...
plt.subplot(2,2,4)
for i in range(len(exclusions_lst)):
    plt.plot(x_range_nm,np.sum(cyt_bc1[i].specD,axis = 0),linewidth=2)
#plt.plot(x_range_nm,np.sum(specR_full,axis = 0),linewidth=5)
#handles = []
#fig.legend(handles, ['n1p1','n1n2','n1p2','p1n2','p1p2','n2p2'], bbox_to_anchor=(2, 0),loc = 'lower right')
